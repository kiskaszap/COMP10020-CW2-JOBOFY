...
function EmployerMyJobs() {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        console.log(jobs); // Assuming response data has a `jobs` array
      })
      .catch((error) => {
  ...
  // View Job - navigate to job details page
  const viewJob = (jobId) => {
    navigate(`/jobs/${jobId}`);
  };
  ...
  const editJob = (jobId) => {
    navigate(`/jobEdit/${jobId}`);
  };

  // Delete Job - send delete request
            ...
            <div className="applied__job__list">
              {jobs.map((job) => (
                <SingleJob
                  ...
                  title={job.title}
                  city={job.city}
                  onView={() => viewJob(job._id)}
                  onEdit={() => editJob(job._id)}
...
// SingleJob component is defined outside of EmployerMyJobs
function SingleJob({ title, city, onView, onEdit, onDelete }) {
  return (
      ...
      <div className="single__applied__job__content">
        <div className="content">
          ...
          <div className="content__info">
            <span>
              <i className="fa-light fa-location-dot" /> {city}
            </span>
          </div>
        </div>
      </div>
      <div className="action">
        <button className="action__btn " id="edit" onClick={onEdit}>
          <svg
            width={19}
            height={19}
            viewBox="0 0 19 19"
        ...
        <button className="action__btn" id="view" onClick={onView}>
          <svg
            width={22}
            height={16}
            viewBox="0 0 22 16"
        ...
        <button className="action__btn" id="delete" onClick={onDelete}>
          <svg
            width={20}
            height={22}
            viewBox="0 0 20 22"
